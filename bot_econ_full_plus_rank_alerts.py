...
USE_UPSTASH_REST = bool(UPSTASH_URL and UPSTASH_TOKEN)
...
async def load_state():
    ...
    data: Optional[Dict[str, Any]] = await STATE_STORE.load()
    if data is None and FALLBACK_STATE_STORE is not None:
        data = await FALLBACK_STATE_STORE.load()

    payload = deserialize_state_payload(data)
    ALERTS = payload.get("alerts", {})
    SUBS = payload.get("subs", {})
    PF = payload.get("pf", {})
    ALERT_USAGE = {int(k): v for k, v in payload.get("alert_usage", {}).items()}

    raw_history = payload.get("news_history", [])
...
async def save_state():
    ...
    if stored or FALLBACK_STATE_STORE is None:
        return
    await FALLBACK_STATE_STORE.save(payload)
