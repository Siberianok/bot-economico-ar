...
def _is_probably_article_url(link: str) -> bool:
    ...
    if any(p.lower() in ARTICLE_SECTION_STOPWORDS for p in parts):
        ...
        return False
    # Requiere un segmento con forma inequívoca de nota (slug, fecha o id)
    def _looks_like_slug(segment: str) -> bool:
        ...
        if "." in seg:
            seg = seg.split(".", 1)[0]
        # necesita guiones con palabras y/o ids
        hyphen_words = seg.split("-") if "-" in seg else []
        if len(seg) >= 12 and len(hyphen_words) >= 3 and any(ch.isdigit() for ch in seg):
            return True
        if len(seg) >= 16 and len(hyphen_words) >= 4:
            return True
        if any(ch.isdigit() for ch in seg) and len(seg) >= 10 and re.search(r"[a-z]", seg):
            return True
        if re.match(r"[a-z0-9]+-[a-z0-9]+-[a-z0-9]+", seg) and len(seg) >= 12:
            return True
        return False
    ...
    if len("".join(parts)) < 14 or len(parts) < 2:
        return False
    return True
